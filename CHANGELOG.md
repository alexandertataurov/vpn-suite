...
- VPS/Remote: stop.sh runs typecheck+lint only (no vitest/playwright); miniapp `test` runs unit project only; `test:all` for full suite. Rule: avoid full:check on resource-constrained VPS.
