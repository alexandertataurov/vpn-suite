...
def _health_state(
    ...
    if handshake_ratio is not None and handshake_ratio < HANDSHAKE_RATIO_DEGRADED:
        return "degraded"
    if (
        peers_max is not None
        and peers_max > 0
        and active_peers >= (peers_max * CAPACITY_DEGRADED_PCT / 100)
    ):
        return "degraded"
        ...
        if hb:
            ...
            if peers_list:
                with_handshake = sum(
                    ...
                    for p in peers_list
                    if (
                        p.get("last_handshake_age_sec") is not None
                        and (p.get("last_handshake_age_sec") or 0) <= 180
                    )
                )
                handshake_ratio = with_handshake / len(peers_list) if peers_list else None
        peers_max = None  # could come from snapshot or config
        peers_fullness_pct = (
            (100.0 * peer_count / peers_max) if peers_max and peers_max > 0 else None
        )
        ...
        if ts_utc:
            try:
                _ = (
                    ...
                    else (ts_utc.isoformat() if hasattr(ts_utc, "isoformat") else str(ts_utc))
                )
            except Exception:
                pass
...
async def build_vpn_node_detail(
    ...
    if peers_list:
        with_handshake = sum(
            1
            for p in peers_list
            if isinstance(p, dict) and (p.get("last_handshake_age_sec") or 0) <= 180
        )
    ...
    public_ip = (
        (server.api_endpoint or "").split("//")[-1].split("/")[0].split(":")[0]
    ...
    system = VpnNodeSystemOut(
        cpu_pct=cpu_pct,
        ram_pct=float(snap["ram"]) if isinstance(snap.get("ram"), int | float) else None,
    )
