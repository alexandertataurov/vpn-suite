...
- Added runtime CSS parity coverage for typography and breakpoints.
  ...
  Affects: `src/design-system/core/tokens/__tests__/token-parity.test.ts`, Storybook foundations audit, `design:check`.
  Migration: run `pnpm run test -- src/design-system/core/tokens/__tests__/token-parity.test.ts` after token or CSS edits.
