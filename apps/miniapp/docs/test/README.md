...
├── design-consistency.test.ts # Design-system policy guards (CI)
...
Tests are colocated with source (`*.test.ts` / `*.test.tsx`). Vitest excludes `src/design-system/**` and `src/lib/telegram/**` from coverage. See `vitest.config.ts` for thresholds.
