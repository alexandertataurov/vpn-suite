...
test.describe("Miniapp Responsive Layout", () => {
  ...
  test("real routes keep CTA visibility and avoid overflow across required widths", async ({ page }) => {
    await injectTelegram(page);
    await mockApi(page);

    for (const viewport of RESPONSIVE_VIEWPORTS) {
      await page.setViewportSize({ width: viewport.width, height: viewport.height });
      for (const pageConfig of REAL_ROUTES) {
        await goToResponsivePath(page, pageConfig.path);
        await waitForResponsiveShell(page);
        await assertPrimaryCtaVisible(page, pageConfig.readyText);
  ...
  test("mock sandboxes keep overflow and touch targets clean across required widths", async ({ page }) => {
    await injectTelegram(page);
    await mockApi(page);

    for (const viewport of RESPONSIVE_VIEWPORTS) {
      await page.setViewportSize({ width: viewport.width, height: viewport.height });
      for (const pageConfig of MOCK_ROUTES) {
        await goToResponsivePath(page, pageConfig.path);
        await waitForResponsiveShell(page);
        await assertNoHorizontalOverflow(page);
        await assertNoTextClipping(page);
