...
Usage:
    python apps/admin-api/scripts/alert_issue_auto_solver.py ERRORS_TG.md > issues.md
...
- If intentionally idle: consider relaxing the threshold further (e.g. 4h) in `infra/monitoring/config/alert_rules.yml` and reload Prometheus.
...
**Proposed actions (see `ERRORS_TG.md` Phase 5 and `infra/monitoring/config/alert_rules.yml`)**
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print(
            "Usage: python apps/admin-api/scripts/alert_issue_auto_solver.py ERRORS_TG.md",
            file=sys.stderr,
        )
        return 1
