...
No native `/metrics`. Metrics come from **wg-exporter** on the host. See `[[07-docs/observability/services/wg-exporter|wg-exporter.md]]`.
...
`transfer_rx` / `transfer_tx` reset to 0 on interface/container restart. Use `rate()` / `increase()` for continuous metrics; see [[07-docs/observability/target-architecture|target-architecture.md]] §8.
