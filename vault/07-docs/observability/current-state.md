...
Operator dashboard: Frontend calls `GET /api/v1/overview/operator`; backend uses `TELEMETRY_PROMETHEUS_URL` for Prometheus queries and Redis for timeseries. If Redis is empty, backend returns one synthetic point. See [[07-docs/observability/runbook-observability|runbook-observability.md]] for troubleshooting "no data".
