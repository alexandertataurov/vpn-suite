...
See [[07-docs/ADMIN-PAGE-MAP|ADMIN-PAGE-MAP.md]] for full UI and API map.
...
- **Attach:** Referrer’s link is `ref_<user_id>`. When a new user opens with `?ref=XXX`, attribution is recorded via `POST /webapp/referral/attach` (Bearer) with `referral_code`; backend creates `Referral(referrer_user_id, referee_user_id, status=pending)`. Frontend uses a [[07-docs/referral-pipeline|cascade capture and sessionStorage]] so ref survives router/Telegram URL changes; backend returns status `attached` | `already_attached` | `self_referral_blocked` | `invalid_ref`.
...
**Source of truth:** [[07-docs/spec-pack/product/VPN-SUITE-BUSINESS-LOGIC-USERFLOWS-SPEC-V2|VPN-SUITE-BUSINESS-LOGIC-USERFLOWS-SPEC-V2.md]] and [[07-docs/spec-pack/product/GROWTH-MONETIZATION-SPEC|GROWTH-MONETIZATION-SPEC.md]].
...
- [[07-docs/referral-pipeline|Referral pipeline (production)]] — Capture cascade, attach API contract, backend rules, observability
...
- [[07-docs/revenue-engine-funnel|docs/revenue-engine-funnel.md]] — Funnel flow and copy
- [[07-docs/MINIAPP_SPEC|docs/MINIAPP_SPEC.md]] — Mini App screens, data flow, telemetry
- [[07-docs/ADMIN-PAGE-MAP|docs/ADMIN-PAGE-MAP.md]] — Admin routes, layout, API list
