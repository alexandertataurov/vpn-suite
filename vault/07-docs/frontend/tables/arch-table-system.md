...
- Provides optional `style` for scroll container (e.g. maxHeight).

### 2) `Table<T>` (standard renderer)

- Renders HTML `<table>` inside `TableContainer`.
...
- Same `TableContainer` wrapper, `TableCell` utilities, sorting UI, density tokens.
...
- `TableContainer` sets container borders, radius, background.
...
- `height` or `maxHeight` on `TableContainer` (via style) to enable scrolling.
