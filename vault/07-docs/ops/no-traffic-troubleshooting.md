...
**Check:** Compare `docker exec <container> wg show awg0` with the issued config. The server shows `h1: N, h2: N, ...`; the client config must have matching `H1 = N`, etc. If the server was started with empty `AWG_H1`/`AWG_H2`/… (e.g. amnezia-awg), it typically uses **h1: 1, h2: 2, h3: 3, h4: 4**. Generated configs use the same defaults; reissue if you have an old config with different H values. See [[07-docs/ops/amneziawg-obfuscation-runbook|amneziawg-obfuscation-runbook.md]].
...
- **Full checklist:** See [[07-docs/ops/server-prerequisites-reconcile|server-prerequisites-reconcile.md]] (port, forward, NAT, routes, MTU).
...
| Symptom | Focus |
|--------|--------|
| ❌ No handshake at all | Key, endpoint, firewall, obfuscation (see [[07-docs/ops/amneziawg-obfuscation-runbook|amneziawg-obfuscation-runbook.md]]). |
