...
Follow the main [[07-docs/security/hardening#1-secrets--auth|Hardening Guide]], then apply on the reference host:
...
From [[07-docs/security/hardening#2-network|Hardening Guide — Network]]:
...
From [[07-docs/security/hardening#3-host-os|Hardening Guide — Host OS]]:
...
2. Configure `.env` and secrets per [[07-docs/ops/required-secrets|required-secrets.md]], then run `./infra/scripts/ops/harden-secrets.sh`.
...
7. Establish a **reference backup & rollback rehearsal**:
   - Take a fresh DB backup: `./manage.sh backup-db` (see [[07-docs/ops/runbook|runbook.md]] and [[07-docs/ops/postgres-safeguards|postgres-safeguards.md]]).
