...
- **Request validation errors (422)** now return the unified error envelope.
  - Evidence: `apps/admin-api/app/main.py`, `apps/admin-api/app/core/error_responses.py`, `apps/admin-api/app/core/error_log_rate_limiter.py`.
...
- Audit middleware attaches actor/context for mutating operations.
  - Evidence: `apps/admin-api/app/core/request_logging_middleware.py`, `apps/admin-api/app/core/audit_middleware.py`.
...
- **Admin API:** `/metrics` (Prometheus format)
  - Core metrics in `apps/admin-api/app/core/metrics.py` include:
...
- **Node-agent:** `/metrics` exposes agent reconciliation + docker exec metrics.
  - Evidence: `apps/node-agent/agent.py`
- **Bot:** `/metrics` for bot health and internal metrics.
  - Evidence: `apps/telegram-bot/main.py`, `apps/telegram-bot/metrics.py`
...
- OpenTelemetry tracing is supported and can be enabled via `OTEL_TRACES_ENDPOINT`.
  - Evidence: `apps/admin-api/app/core/otel_tracing.py`, `apps/admin-api/app/core/config.py`.
