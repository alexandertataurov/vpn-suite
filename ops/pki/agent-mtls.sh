...
init_ca() {
  ...
  if [ -f "$CA_KEY" ] || [ -f "$CA_CERT_PEM" ]; then
    echo "CA already exists: $CA_KEY / $CA_CERT_PEM" >&2
    exit 2
  fi
  ...
  echo "Wrote CA key:  $CA_KEY"
  echo "Wrote CA cert: $CA_CERT_PEM"
}
...
issue_client() {
  ...
  if [ -z "$server_id" ] || [ -z "$out_dir" ]; then
    echo "Usage: $0 issue-client <server_id> <out_dir>" >&2
    exit 2
  fi
  if [ ! -f "$CA_KEY" ] || [ ! -f "$CA_CERT_PEM" ]; then
    echo "CA missing. Run: $0 init-ca" >&2
    exit 2
  fi
  mkdir -p "$out_dir"
  ...
  echo "Wrote client key:  $key"
  echo "Wrote client cert: $crt"
  echo
  echo "Node: place these into amnezia/amnezia-awg2/secrets/ as:"
  echo "  agent_client_key.pem"
  echo "  agent_client_cert.pem"
}
...
case "$cmd" in
  ...
  *)
    echo "Usage: $0 {init-ca|issue-client <server_id> <out_dir>}" >&2
    exit 1
    ;;
esac
