...
All tables in the admin use the DataTable design system. Use `Table` for declarative column-based data, `DataTable` for custom layouts, and `TableSkeleton` for loading states.
...
Use for raw `<table>` with custom rows (e.g. Servers, Users):

```tsx
import { DataTable } from "@vpn-suite/shared/ui";

<DataTable ref={scrollRef} maxHeight={480}>
