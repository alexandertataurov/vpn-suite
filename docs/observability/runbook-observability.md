...
1. From repo root, start monitoring stack:
   ...
   - Or: `docker compose -f docker-compose.yml -f docker-compose.observability.yml --profile monitoring up -d discovery-runner prometheus grafana cadvisor node-exporter loki promtail tempo otel-collector`
...
8. Trace validation (optional): generate a few requests and confirm Tempo counters increase (see [verification.md](verification.md) §5).
9. Alerting: Alertmanager is running, but `config/monitoring/alertmanager.yml` uses a no-op receiver by default. Configure a receiver (Slack/Email/PagerDuty) before relying on notifications.

## Debug

- Prometheus targets: `curl -s http://127.0.0.1:19090/api/v1/targets | rg -n "node-agent|admin-api"`
- Discovery runner: `docker logs --tail=200 vpn-suite-discovery-runner-1`, `cat config/monitoring/discovery/targets.json`
