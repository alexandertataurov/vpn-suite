...
- Token source: `frontend/admin/src/design-system/tokens.css` (admin). Miniapp: `frontend/miniapp/src/design-system/styles/tokens/base.css`, `frontend/miniapp/src/design-system/styles/theme/consumer.css`, `frontend/miniapp/src/design-system/styles/theme/telegram.css`.

## Miniapp design-system foundations

- **Token stack (canonical)**:
  - Base tokens: `frontend/miniapp/src/design-system/styles/tokens/base.css`.
  - Consumer themes: `frontend/miniapp/src/design-system/styles/theme/consumer.css` (`html[data-theme="consumer-dark" | "consumer-light"]`).
  - Telegram bridge: `frontend/miniapp/src/design-system/styles/theme/telegram.css` (`--ds-*` vars mapped onto Telegram `--tg-theme-*` + consumer tokens).
  - Layout and shell: `frontend/miniapp/src/design-system/styles/layout/zones.css`, `frontend/miniapp/src/design-system/styles/shell/frame.css`.
- **Typography**:
  - **Canonical scale**: `--typo-*` tokens (display, h1–h4, body, body-sm, caption, meta) defined in consumer theme. Use these for all font-size decisions in product CSS and components.
  - Legacy size primitives `--text-xs`, `--text-sm`, `--text-base`, `--text-lg` exist in consumer/base for compatibility; prefer `--typo-*` where the size matches (e.g. `--text-xs` → `--typo-caption-size` or `--typo-meta-size`, `--text-lg` → `--typo-h2-size`, `--text-base` → `--typo-h3-size`). New CSS should use `--typo-*` only.
  - Miniapp aliases: `--ds-font-*` tokens in `telegram.css` and `TYPOGRAPHY_TOKENS` in `frontend/miniapp/src/design-system/tokens/typography.ts`.
  - **Rule**: in miniapp code, use `Display/H1/H2/H3/Body/Caption` components or `--typo-*` / `--ds-font-*` tokens; do **not** hardcode `font-size` except inside Storybook-only showcase stories.
- **Color**:
  - Use `--color-*` semantic tokens from `base.css` and `consumer.css` (bg, surface, surface-2, text, text-muted, text-tertiary, border, border-subtle, accent, success, warning, error, info).
  - Telegram theme overrides flow through `--ds-color-*` tokens; app code should not read `--tg-theme-*` directly.
  - **Rule**: no raw hex/RGB colors in miniapp TS/TSX/CSS; always bind to an existing `--color-*` or `--ds-color-*` token.
- **Spacing, radius, motion**:
  - Spacing: `--spacing-*` and `--space-*` tokens from `base.css` plus miniapp layout vars like `--miniapp-page-gap`, `--miniapp-section-gap`, `--miniapp-card-padding` from `zones.css`.
  - Radius: `--radius-*` + miniapp-specific `--miniapp-radius-*` from `consumer.css`.
  - Motion: `--duration-*` and `--ease-*` tokens from `base.css` (and `MOTION_TOKENS` in TS).
  - **Rule**: use spacing/radius/motion tokens only; no ad-hoc `margin: 14px` / `border-radius: 10px` / `transition: 0.2s` in app CSS.
- **Layout contract**:
  - Viewport + safe areas: `zones.css` owns `--app-height`, `--safe-*`, `--miniapp-*` layout vars; Telegram WebView insets are normalized there.
  - Shell: `.miniapp-shell`, `PageFrame`, `PageSection`, hero and mission/home patterns define the only allowed page scaffolding.
  - **Rule**: miniapp pages should compose `PageFrame` + design-system patterns/components; avoid bespoke page wrappers with their own spacing/grid systems.

### Miniapp lint/guardrail recommendations

- **TypeScript / TSX**:
  - Disallow `style={{ color: "#...", background: "rgb(...)" }}` and other hardcoded color/spacing values in miniapp app code; allow in Storybook-only stories when needed for visualization.
  - Prefer design-system exports (`Button`, `Field`, `SettingsCard`, `Mission*`, `Page*` layouts, `EmptyStateBlock`, `PageStateScreen`, etc.) over raw `<button>`, `<input>`, and ad hoc layout wrappers for product UI.
  - Enforce that new miniapp components live under `frontend/miniapp/src/design-system` when they are reused across pages; page files in `frontend/miniapp/src/pages` should mostly compose existing primitives/patterns.
- **CSS**:
  - Restrict new miniapp CSS to design-system layers (`styles/tokens`, `styles/theme`, `styles/layout`, `styles/shell`, `styles/content`); avoid page-scoped CSS files outside the design-system tree.
  - Ban new raw hex/RGB colors and magic `font-size` / `margin` / `padding` literals in miniapp CSS; require usage of existing `--color-*`, `--typo-*`, `--spacing-*`, and `--miniapp-*` layout tokens instead.
  - For new patterns, add both CSS (under `styles/content`) and a Storybook story that documents the pattern as the spec before wiring it into a page.
