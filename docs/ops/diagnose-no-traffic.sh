...
docker exec "$AWG_CONTAINER" ss -lunp 2>/dev/null | grep -E ":$WG_PORT|$WG_PORT " || true
...
docker exec "$AWG_CONTAINER" iptables -t nat -S POSTROUTING 2>/dev/null | grep -E "10\.|MASQUERADE" | head -20
...
docker exec "$AWG_CONTAINER" ip route 2>/dev/null | grep -E "10\.|$IFACE" || true
