 ...
 * Emits telemetry event "web_vital" once per metric per page load.
 */
...
type VitalUnit = "ms" | "score";

const sent = new Set<VitalName>();

function send(name: VitalName, value: number, unit: VitalUnit, extra?: Record<string, unknown>): void {
...
export function initWebVitals(): void {
  ...
  // INP (best-effort): max interaction event duration.
  let inpValue = 0;
  try {
