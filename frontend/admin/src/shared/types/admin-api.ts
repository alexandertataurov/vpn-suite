...
export interface AppSettingsCapabilitiesOut {
  can_read: boolean;
  can_write: boolean;
  can_manage_dangerous: boolean;
  env_editor_enabled: boolean;
  can_edit_env: boolean;
  can_cleanup_db: boolean;
}
...
export interface AppSettingsOut {
  ...
  /** Capability flags used to gate dangerous settings controls in UI. */
  capabilities?: AppSettingsCapabilitiesOut;
}
...
export interface AwgDownloadLinkResponse {
  url: string;
  expires_in: number;
}
...
export interface ServerTelemetryOut {
  ...
  /** AmneziaWG obfuscation from node (wg show). Current H1–H4 on node. */
  node_obfuscation?: NodeObfuscationOut | null;
}

export interface NodeObfuscationOut {
  H1?: number | null;
  H2?: number | null;
  H3?: number | null;
  H4?: number | null;
