...
import { ModernHeroCard } from "@/design-system/compositions/recipes";
import { IconShield } from "@/design-system/icons";
import { BREAKPOINT_PX, BREAKPOINT_TOKENS } from "@/design-system/core/tokens";
import { StoryCard, StoryPage, StorySection, TokenTable, UsageExample, ValuePill } from "@/design-system/utils/story-helpers";
...
function BreakpointShell({ width }: { width: number }) {
  ...
  return (
    <div style={{ ...shellStyle, gridTemplateColumns: wide ? "minmax(0, 1.2fr) minmax(240px, 0.8fr)" : "minmax(0, 1fr)" }}>
      <ModernHeroCard
        status={wide ? "warning" : "active"}
        icon={<IconShield size={36} strokeWidth={wide ? 2 : 3} />}
        title={wide ? "Tunnel active" : "Protected"}
        description={wide ? "Traffic is protected, but latency is elevated and route quality is degraded." : "Traffic is protected through the fastest healthy route."}
      />
