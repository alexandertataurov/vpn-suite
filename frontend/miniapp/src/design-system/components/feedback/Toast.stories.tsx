...
import { Button, InlineAlert, Toast, ToastContainer, ToastViewport, useToast } from "@/design-system/components";
...
const STATIC_TOASTS = [
  { id: "success", variant: "success", message: "Config updated.", duration: 4000, dismissible: false },
  { id: "error", variant: "error", message: "Connection failed.", duration: 8000, dismissible: true },
  { id: "info", variant: "info", message: "New server region available.", duration: 5000, dismissible: false },
...
export const Default: Story = {
  render: () => (
        <StoryPage
          eyebrow="Components"
          title="Toast"
          summary="Toast is the ephemeral feedback layer for mobile. It renders below the header zone, spans the content width, uses a restrained compact surface with light status tinting, and supports auto-dismiss, persistent messages, and a maximum three-item stack."
      stats={[
    ...
    >
      ...
      >
        <ThreeColumn>
              <StoryCard title="All variants" caption="Success, error, info, and persistent share one compact shell with quiet status accents, lighter chrome, and consistent dismiss behavior.">
          ...
          <UsageExample title="Stacking and dismissal" description="Newest toasts appear on top. The stack never exceeds three visible items, and persistent or error toasts keep a dismiss control.">
            <div className="toast-story-contract">
              ...
              <div className="toast-story-contract-row">
                ...
                <span>Newest on top, visually stacked by default to save space, maximum 3 visible. Hover or focus expands the stack back to a 4px gap.</span>
              </div>
              <div className="toast-story-contract-row">
      ...
      >
        <TwoColumn>
          <UsageExample title="Motion contract" description="Entry slides down and fades in. Exit slides up and fades out, while a hairline progress line communicates the remaining auto-dismiss time.">
            <div className="toast-story-static-stack">
              <Toast variant="info" message="New server region available." duration={5000} />
              <Toast variant="error" message="Connection failed." dismissible duration={8000} className="toast-exit" />
            </div>
          </UsageExample>
          <UsageExample title="Reduced motion" description="Under `prefers-reduced-motion`, toast entry and exit become fade-only and the progress indicator stays static.">
            <div className="toast-story-contract">
              ...
              <div className="toast-story-contract-row">
                ...
                <span>Bottom hairline animates for auto-dismissed toasts and pauses on hover or focus. Persistent toasts omit the bar.</span>
              </div>
            </div>
          </UsageExample>
        </TwoColumn>
