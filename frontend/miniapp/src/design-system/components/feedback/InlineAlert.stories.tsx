...
import { StoryCard, StoryPage, StorySection, ThreeColumn, UsageExample } from "@/design-system/utils/story-helpers";
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: "Inline alert with restrained surface tinting, a compact status dot, calm title hierarchy, and an optional aligned action row.",
      },
    },
  },
  argTypes: {
...
export const Default: Story = {
  render: () => (
    <StoryPage
      eyebrow="Components"
      title="Inline alert"
      summary="Inline alerts are mobile-first status blocks. The dot, title, body, and optional actions all align to one grid while the shell stays compact, subdued, and easy to scan across passive, warning, error, and actionable states."
      stats={[
    ...
    >
      <StorySection
        ...
        description="All variants share the same compact shell, soft tint, and title-anchored status dot. Only the tone changes."
      >
        <ThreeColumn>
          <StoryCard title="Status updates" caption="Info and success stay contextual instead of interruptive.">
            <Stack gap="2">
              <InlineAlert variant="info" title="New node available in your region." />
              <InlineAlert variant="success" title="Traffic is now protected." />
            </Stack>
          </StoryCard>
          <StoryCard title="Attention states" caption="Warning and error need clearly different tints so they do not collapse into the same warm beige.">
            <Stack gap="2">
              <InlineAlert variant="warning" title="Subscription expires in 3 days." />
              <InlineAlert variant="error" title="Config is no longer valid." body="Download a fresh profile from Devices." />
            </Stack>
          </StoryCard>
            ...
            <InlineAlert
              ...
              actions={
                <>
                  ...
                  <Button variant="primary" size="sm">Reconnect</Button>
                </>
              }
            />
          </StoryCard>
