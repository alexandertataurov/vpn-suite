...
export type { RefCaptureSource } from "@/lib/referralCapture";
...
/** In-memory guard so attach runs only once per session (Strict Mode / remount safe). */
...
export function useReferralAttach(): void {
  ...
  useEffect(() => {
    if (!token || attachAttempted) return;
