...
- **Consumer theme:** `frontend/miniapp/src/design-system/styles/theme/consumer.css` (`html[data-theme="consumer-dark" | "consumer-light"]`)
...
- Use `--typo-*` or `--ds-font-*` tokens for size and weight. Miniapp aliases live in `telegram.css` and `frontend/miniapp/src/design-system/tokens/typography.ts`.
...
- [ ] New miniapp components that are reused across pages live under `frontend/miniapp/src/design-system`; pages in `src/pages` mostly compose existing primitives/patterns.
