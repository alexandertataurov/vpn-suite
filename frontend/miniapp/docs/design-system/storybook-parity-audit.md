...
- Placeholder routes in onboarding harness
  - Resolved: OnboardingPage.stories uses OnboardingSandbox with real OnboardingPage and ViewportShellRoutes.
...
- Fail when a page story route path differs from `AppRoutes` (per-page stories in `design-system/stories/pages/`).
