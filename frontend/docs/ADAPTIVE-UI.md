...
Defined in `admin/src/styles/admin-content-aware.css`.

---

## Breakpoints (single source of truth)

Defined in `shared/src/theme/breakpoints.css` and used in `admin/src/styles/admin-adaptive-vars.css`.
...
Semantic tokens in `tokens.css`; theme map in `shared/docs/theme-token-map.md`. Telemetry severity: pair **color + icon** (healthy=green+check, warning=amber+warning, critical=red+alert, unknown=gray+minus).

## Tests and a11y

- **Playwright:** `admin/e2e/adaptive-ui.spec.ts` — viewports 360, 640, 1024, 1440; no horizontal scroll; sidebar overlay at XS, persistent at MD; Servers cards at XS; skip link.
- **A11y:** Keyboard nav, `:focus-visible`, ARIA landmarks, 44px min touch targets on XS/SM. See `docs/ADAPTIVE-UI-TEST-PLAN.md`.
