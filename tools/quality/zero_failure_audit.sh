...
compose_audit() {
  env ENV_FILE="$ENV_FILE" docker compose --env-file "$ENV_FILE" \
    -p "$AUDIT_PROJECT" \
    -f infra/compose/docker-compose.audit.yml \
    "$@"
}
...
(cd apps/admin-api && NODE_MODE=mock NODE_DISCOVERY=agent pytest -q) \
...
phase "Run API Audit"
python3 tools/quality/zero_failure_api_audit.py \
  --base-url "$BASE_API" \
  --out-dir "$OUT_DIR" \
...
if [[ -d apps/admin-web/playwright-report ]]; then
  cp -R apps/admin-web/playwright-report "$OUT_DIR/playwright/playwright-report"
fi
if [[ -d apps/admin-web/test-results ]]; then
  cp -R apps/admin-web/test-results "$OUT_DIR/playwright/test-results"
fi

phase "Static Production Safety Scans"
rg -n --glob '!**/node_modules/**' 'console\.log\(' apps/admin-web/src apps/shared-web/src apps/admin-api/app apps/telegram-bot \
  > "$OUT_DIR/scans/console_log_hits.txt" || true
rg -n --glob '!**/node_modules/**' '(/debug|debug[_-]|test-only|__debug__)' apps/admin-api/app apps/admin-web/src apps/telegram-bot \
  > "$OUT_DIR/scans/debug_surface_hits.txt" || true

if [[ -d apps/admin-web/dist ]]; then
  rg -n 'SECRET_KEY|POSTGRES_PASSWORD|BOT_TOKEN|TELEGRAM_STARS_WEBHOOK_SECRET|DATABASE_URL|AGENT_SHARED_TOKEN' apps/admin-web/dist \
    > "$OUT_DIR/scans/frontend_secret_leak_hits.txt" || true
fi
if [[ -d apps/miniapp/dist ]]; then
  rg -n 'SECRET_KEY|POSTGRES_PASSWORD|BOT_TOKEN|TELEGRAM_STARS_WEBHOOK_SECRET|DATABASE_URL|AGENT_SHARED_TOKEN' apps/miniapp/dist \
