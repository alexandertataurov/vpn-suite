...
if [[ "${VERIFY_SKIP_DB:-0}" != "1" ]]; then
  ...
  (cd apps/admin-api && python3 -m alembic upgrade head)
  step "Backend: Migration integrity (downgrade/upgrade)"
  (cd apps/admin-api && python3 -m alembic downgrade -1 && python3 -m alembic upgrade head)
  step "Backend: Alembic check"
  (cd apps/admin-api && python3 -m alembic check)
  step "Backend: Pytest"
  (cd apps/admin-api && NODE_MODE=mock TELEGRAM_STARS_WEBHOOK_SECRET="" python3 -m pytest tests/ -v)
  step "Bot: Pytest"
  (cd apps/telegram-bot && python3 -m pytest -q)
else
