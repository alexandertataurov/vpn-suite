...
python3 -m infra.discovery.runtime --out-dir "$OUT_DIR" 2>/dev/null || true
...
if rg -i 'amnezia-awg' infra/discovery/runtime/*.py 2>/dev/null | grep -v 'image\|#\|pattern' | grep -q .; then
