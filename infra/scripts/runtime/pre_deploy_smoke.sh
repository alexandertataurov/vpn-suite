...
# Usage: ./infra/scripts/runtime/pre_deploy_smoke.sh [2min|10min]   (default: 2min)
set -euo pipefail
IFS=$'\n\t'
...
run_2min() {
  if [[ "$SKIP_CHECK" != "1" ]]; then
    ...
    if ./manage.sh check 2>/dev/null; then
      ...
      (cd apps/admin-api && ruff check . -q 2>/dev/null) || true
      (cd "$ROOT/../.." && pnpm run lint 2>/dev/null) || true
    fi
  fi
...
run_10min() {
  if [[ "$SKIP_CHECK" != "1" ]]; then
    ...
    if ./manage.sh verify 2>/dev/null; then
      ...
      (cd apps/admin-api && ruff check . -q && python3 -m pytest tests/ -q) || true
      (cd "$ROOT/../.." && pnpm run lint && pnpm run typecheck && pnpm run test -- --run) || true
