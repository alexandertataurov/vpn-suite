...
# Usage: ./infra/scripts/runtime/sanitize-local-db.sh
set -euo pipefail
IFS=$'\n\t'
cd "$(dirname "$0")/../../.."
...
log "Sanitization done. Restart admin-api: docker compose -f infra/compose/docker-compose.yml restart admin-api"
