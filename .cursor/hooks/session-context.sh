...
CONTEXT="VPN Suite (this repo): Full-stack monorepo. Infra: Docker Compose, GitHub Actions CI, self-hosted staging. Backend: Python 3.12, FastAPI, SQLAlchemy async, Alembic, Redis, PostgreSQL (apps/admin-api/). Frontend: React 18, TypeScript, pnpm workspaces admin, miniapp, shared (apps/admin-web/, apps/miniapp/, apps/shared-web/). Bot: Python 3.12 + aiogram (apps/telegram-bot/). Node-agent for VPN hosts (apps/node-agent/). Ops: manage.sh, config/caddy, config/monitoring. Apply team-contract and .cursor/rules.md; use PROMPT.md workflow and domain subagents for cross-boundary work."
